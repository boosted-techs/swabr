...
$(document).ready(function(){
    ...
    $("#createItem").click(function(){
        $("#itemsListDiv").toggleClass("w-50", "col-md-12");
    ...
    $("#gen4me").click(function(){
        ...
        if($("#gen4me").prop("checked")){
            ...
            do{
                ...
                let rand = Math.random().toString(36).slice(2).substring(0, 10).toUpperCase();
    ...
    $("#addNewItem").click(function(e){
        ...
        let itemPrice = $("#itemPrice").val().replace(",", "");
        let itemCode = $("#itemCode").val();
        let dateAdded = $("#dateAdded").val()
        ...
        $.ajax({
            ...
            data:{itemName:itemName, dateAdded : dateAdded, supplier : supplier, itemQuantity:itemQuantity, itemPrice:itemPrice, itemDescription:itemDescription, itemCode:itemCode},
    ...
    $("#itemsListTable").on('click', ".editItem", function(e){
        ...
        let itemDesc = $("#itemDesc-"+itemId).attr('title');
        let itemName = $("#itemName-"+itemId).html();
        let itemPrice = $("#itemPrice-"+itemId).html().split(".")[0].replace(",", "");
        let itemCode = $("#itemCode-"+itemId).html();
    ...
    $("#editItemSubmit").click(function(){
        ...
        let itemDesc = $("#itemDescriptionEdit").val();
        let itemId = $("#itemIdEdit").val();
        let itemCode = $("#itemCodeEdit").val();
    ...
    $("#itemsListTable").on('click', '.updateStock', function(){
        ...
        let itemId = $(this).attr('id').split("-")[1];
        let itemName = $("#itemName-"+itemId).html();
        let itemCurQuantity = $("#itemQuantity-"+itemId).html();
        let itemCode = $("#itemCode-"+itemId).html();
        let itemSupplier = $("#itemSupplier-" + itemId).html()
        let itemDateOfStock = $("#itemStockDate-" + itemId).html()
        ...
        $("#supplierUpdate").val(itemSupplier)
        $("#dateOfSupplyUpdate").val(itemDateOfStock)
    ...
    $("#stockUpdateType").on('change', function(){
        let updateType = $("#stockUpdateType").val();
    ...
    $("#stockUpdateSubmit").click(function(){
        let updateType = $("#stockUpdateType").val();
        let stockUpdateQuantity = $("#stockUpdateQuantity").val();
        let stockUpdateDescription = $("#stockUpdateDescription").val();
        let itemId = $("#stockUpdateItemId").val();

        let supplier = $("#supplierUpdate").val()
        let itemDateOfStock = $("#dateOfSupplyUpdate").val()
        ...
        $.ajax({
            ...
            data: {_iId:itemId,dateOfStock: itemDateOfStock, supplier:supplier,  _upType:updateType, qty:stockUpdateQuantity, desc:stockUpdateDescription}
        }).done(function(returnedData){
            if(returnedData.status === 1){
    ...
    $("#itemsListTable").on('click', '.delItem', function(e){
        ...
        //get the item id
        let itemId = $(this).parents('tr').find('.curItemId').val();
        let itemRow = $(this).closest('tr');//to be used in removing the currently deleted row
        
        if(itemId){
            let confirm = window.confirm("Are you sure you want to delete item? This cannot be undone.");
 ...
 */
function lilt(url){
    let orderBy = $("#itemsListSortBy").val().split("-")[0];
    let orderFormat = $("#itemsListSortBy").val().split("-")[1];
    let limit = $("#itemsListPerPage").val();
