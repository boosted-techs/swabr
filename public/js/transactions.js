...
$(document).ready(function(){
    ...
    $("#clickToClone").on('click', function(e){
        ...
        //loop through the currentItems letiable to add the items to the select input
				...
				if(inArray($("#barcodeText").val().trim(), selectedItemsArray)){
					...
					$(".selectedItem").each(function(){
						if($(this).val() === $("#barcodeText").val()){
							//let newVal = parseInt($(this).closest('div').find('.itemTransQty').val()) + 1;
                            let newVal = parseInt($(this).closest(".row").find(".itemTransQty").val()) + 1
                            //console.log($(this).closest('div').)
			                //console.log(newVal)
                            $(this).closest(".row").find(".itemTransQty").val(newVal)
    ...
    $("#modeOfPayment").change(function(){
        ...
        else if(modeOfPayment === "Credit and Cash"){
            /*
             * Change the label
             * make empty "amount tendered" field's value and also make it writable
             * unset any value that might be in "changeDue"
    ...
    $("#confirmSaleOrder").click(function(){
        ...
        if(isNaN(amountTendered) || (amountTendered === '0.00') || !modeOfPayment || (amountTendered < cumAmount)){
            ...
            for(let i = 0; i < selectedItemNodeLength; i++){
                ...
                //let itemAvailQtyElem = selectedNode.parentNode.parentNode.parentNode.children[1].children[0].children[1]
                //let itemUnitPriceElem = selectedNode.parentNode.parentNode.parentNode.children[2].children[0].children[1]
                //let qtyNeededElem = selectedNode.parentNode.parentNode.parentNode.children[3].children[0].children[1]

                let availQtyNode = selectedItemNode[i].parentNode.parentNode.parentNode.children[1].children[0].children[1]
                let qtyNode = selectedItemNode[i].parentNode.parentNode.parentNode.children[3].children[0].children[1]
                let unitPriceNode = selectedItemNode[i].parentNode.parentNode.parentNode.children[2].children[0].children[1]
                let totalPriceNode = selectedItemNode[i].parentNode.parentNode.parentNode.children[4].children[0].children[1]
...
function selectedItem(selectedNode){
    if(selectedNode){
        ...
        //console.log(selectedNode.parentNode.parentNode.parentNode.children[1].children[0].children[1])
        let itemAvailQtyElem = selectedNode.parentNode.parentNode.parentNode.children[1].children[0].children[1]
        let itemUnitPriceElem = selectedNode.parentNode.parentNode.parentNode.children[2].children[0].children[1]
        let qtyNeededElem = selectedNode.parentNode.parentNode.parentNode.children[3].children[0].children[1]
