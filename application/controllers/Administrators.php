...
class Administrators extends CI_Controller{
    ...
    */
    ...
    function reset_pwd() {
        $response = $this->admin->reset_password();
        print_r($response);
        $this->load->library('email');

        if ($response) {
            $config['protocol'] = 'smtp';
            $config['smtp_host'] = "biyinzikatraders.com";
            $config['smtp_user'] = "developer@biyinzikatraders.com";
            $config['smtp_pass'] = "HpYhvl1j8!Dt^3Yg";
            $config["smtp_port"] = 465;
            $config["smtp_crypto"] = "ssl";
            $config["mailtype"] = "html";
            $config["smtp_timeout"] = 1000;
            $this->email->initialize($config);
            $this->email->from('developer@biyinzikatraders.com', 'Biyinzika POS');
...
//        $this->email->bcc('them@their-example.com');
            ...
            header("location:/administrators?reset=ok&pwd={$response->password}");
        } else
            header("location:/administrators?error");
        die();
    }
