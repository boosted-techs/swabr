...
class Items extends CI_Controller{
    ...
    */
    ...
    public function lilt(){
        ...
        $limit = $this->input->get('limit', TRUE) ? $this->input->get('limit', TRUE) : 50;//show $limit per page
    ...
    */
    ...
    #[NoReturn] function generate_bar_codes() {
        $this->genlib->superOnly();
        $data['pageTitle'] = "BarCodes";
        $items = $this->item->get_items();
        $data['pageContent'] = $this->load->view('items/barcodes', ['content' => $items], true);
        $this->load->view("main", $data);
    }
    ...
    function update_codes() {
        ...
        foreach ($items as $item) {
            echo $item->code . "<br/>";
            $i = str_replace(" ", "", $item->code);
            $i = $this->remove_first_zero_after_hyphen($i);
            $this->db->where("id", $item->id);
            $this->db->update("items", ['code' => $i]);
        }
        die();
    }

    function remove_first_zero_after_hyphen($str): array|string|null
    {
        return preg_replace('/-0+(?=[1-9])/', '-', $str);
    }
}