...
class Admin extends CI_Model{
    ...
    */
    ...
    function reset_password() {
        ...
        if (empty($user))
            return false;
        $new_password = rand(8, 100) . time();
        $new_password = substr(hash("md5", $new_password), 0, 6);
        ...
        if ($data) {
            $this->db->where("id", $user);
            $this->db->update("admin", ['password' => $password_hash]);
    ...
    */
    ...
    function pwd_reset($password, $old_password): bool
    {
        $user = $this->session->admin_id;
